...
public class RecurringTransactionOccurrenceConfiguration : IEntityTypeConfiguration<RecurringTransactionOccurrence>
{
    public void Configure(EntityTypeBuilder<RecurringTransactionOccurrence> builder)
    {
        ...
        builder.HasOne(occurrence => occurrence.Owner).WithMany().HasForeignKey(occurrence => occurrence.OwnerId).OnDelete(DeleteBehavior.Restrict).HasConstraintName("fk_recurring_transaction_occurrences_users_owner_id");
