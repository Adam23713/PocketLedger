...
public class RecurringTransactionService(PocketLedgerDbContext dbContext, IUserContextService userContext) : IRecurringTransactionService
{
    ...
    public async Task<RecurringTransaction> CreateAsync(RecurringTransaction template, CancellationToken cancellationToken)
    {
        ...
        template.AutomationStartsOn = await userContext.TodayAsync(cancellationToken);
    ...
    public async Task UpdateAsync(RecurringTransaction template, CancellationToken cancellationToken)
    {
        ...
        if (scheduleChanged || !existing.Enabled && template.Enabled) existing.AutomationStartsOn = await userContext.TodayAsync(cancellationToken);
