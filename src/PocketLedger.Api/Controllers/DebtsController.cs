...
public sealed class DebtsController(IDebtService service) : ControllerBase
{
    ...
    [HttpPost] public async Task<IActionResult> Create(DebtWriteRequest request, CancellationToken token) { var created = await service.CreateAsync(request.Debt, request.RecurringPayment, token); return CreatedAtAction(nameof(Get), new { id = created.Id }, created); }
    [HttpPut("{id:guid}")] public async Task<IActionResult> Update(Guid id, DebtWriteRequest request, CancellationToken token) { if (id != request.Debt.Id) return BadRequest(); await service.UpdateAsync(request.Debt, request.RecurringPayment, token); return NoContent(); }
    ...
    [HttpGet("operations/{transactionId:guid}")] public async Task<IActionResult> GetOperation(Guid transactionId, CancellationToken token) => (await service.GetOperationAsync(transactionId, token)) is { } item ? Ok(item) : NotFound();
    [HttpPost("{id:guid}/operations")] public async Task<IActionResult> AddOperation(Guid id, DebtOperationWriteRequest request, CancellationToken token) => Ok(await service.AddOperationAsync(id, request.Operation, token));
    [HttpPut("operations/{transactionId:guid}")] public async Task<IActionResult> UpdateOperation(Guid transactionId, DebtOperationWriteRequest request, CancellationToken token) => Ok(await service.UpdateOperationAsync(transactionId, request.Operation, token));
