...
public sealed class TransactionsController(ITransactionService service) : ControllerBase
{
    ...
    [HttpPost("export-query")] public async Task<IActionResult> ExportQuery(TransactionFilter filter, CancellationToken token) => Ok(await service.GetForExportAsync(filter, token));
    ...
    [HttpPost] public async Task<IActionResult> Create(Transaction item, CancellationToken token) { var created = await service.CreateAsync(item, token); return CreatedAtAction(nameof(Get), new { id = created.Id }, created); }
    [HttpPut("{id:guid}")] public async Task<IActionResult> Update(Guid id, Transaction item, CancellationToken token) { if (id != item.Id) return BadRequest(); await service.UpdateAsync(item, token); return NoContent(); }
