...
public sealed class TransactionsController(ITransactionService service) : ControllerBase
{
    ...
    [HttpPost, ProducesResponseType<TransactionDto>(StatusCodes.Status201Created)] public async Task<IActionResult> Create(TransactionCreateRequest request, CancellationToken token) { var created = await service.CreateAsync(request.ToInput(), token); return CreatedAtAction(nameof(Get), new { id = created.Id }, created.ToDto()); }
    [HttpPut("{id:guid}")] public async Task<IActionResult> Update(Guid id, TransactionUpdateRequest request, CancellationToken token) { await service.UpdateAsync(id, request.ToInput(), token); return NoContent(); }
