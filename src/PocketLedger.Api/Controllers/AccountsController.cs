...
public sealed class AccountsController(IAccountService service) : ControllerBase
{
    ...
    [HttpPost] public async Task<IActionResult> Create(AccountDto item, CancellationToken token) { var created = await service.CreateAsync(item.ToEntity(), token); return CreatedAtAction(nameof(Get), new { id = created.Id }, created.ToDto()); }
    [HttpPut("{id:guid}")] public async Task<IActionResult> Update(Guid id, AccountUpdateRequest request, CancellationToken token) { if (id != request.Account.Id) return BadRequest(); await service.UpdateAsync(request.Account.ToEntity(), request.CreateInitialBalanceAdjustment, token); return NoContent(); }
    ...
    [HttpGet("{id:guid}/recent-transactions")] public async Task<IActionResult> RecentTransactions(Guid id, [FromQuery] int count = 10, CancellationToken token = default) => Ok((await service.GetRecentTransactionsAsync(id, count, token)).Select(ApiContractMapper.ToDto));
}
