document.querySelectorAll("[data-money-value]").forEach(display => {
    ...
    function parse(value, format) {
        const negative = display.dataset.moneyAllowNegative === "true" && value.trimStart().startsWith("-");
        if (!/\d/.test(value)) return { integer: "", fraction: "", hasDecimal: false, negative };
        ...
        return { integer, fraction, hasDecimal: decimalIndex >= 0 && format.decimalPlaces > 0, negative };
    }

    function render(parts, format) {
        const integer = parts.integer.replace(/\B(?=(\d{3})+(?!\d))/g, format.thousandsSeparator);
        return (parts.negative ? "-" : "") + integer + (parts.hasDecimal ? format.decimalSeparator + parts.fraction : "");
    }
    ...
    function formatInput() {
        ...
        canonical.value = (parts.negative && parts.integer ? "-" : "") + parts.integer + (parts.fraction ? "." + parts.fraction : "");
    ...
    function renderCanonical() {
        ...
        const format = options();
        const fixed = display.dataset.moneyFixedDecimals === "true";
        display.value = render({ negative: display.dataset.moneyAllowNegative === "true" && value.startsWith("-"), integer: integer.replace(/\D/g, "") || "0", fraction: (fixed ? fraction.padEnd(format.decimalPlaces, "0") : fraction).slice(0, format.decimalPlaces), hasDecimal: (fixed || fraction.length > 0) && format.decimalPlaces > 0 }, format);
    }

    display.addEventListener("input", formatInput);
    display.addEventListener("money:refresh", renderCanonical);
    if (display.dataset.moneyFixedDecimals === "true") display.addEventListener("blur", renderCanonical);
