...
public record PlannerItemInput(DateOnly Month, DateOnly? PlannedDate, TransactionType Type, Guid AccountId, Guid? TargetAccountId, Guid? CategoryId, decimal Amount, string Currency, decimal AccountAmount, decimal? TargetAmount, string? Note, bool IsPaused = false);
public enum PlannerEventSource { Recurring = 1, Planned = 2 }
public record PlannerEvent(Guid Id, PlannerEventSource Source, TransactionType Type, DateOnly? Date, Guid? AccountId, string AccountName, Guid? TargetAccountId, string? TargetAccountName, string? CategoryName, string? Note, decimal Amount, string Currency, decimal AccountAmount, string AccountCurrency, decimal? TargetAmount, string? TargetCurrency, decimal SourceChange, decimal TargetChange, TransactionReportingClassification Classification, bool IsPaused = false, string? CategoryIconPath = null, string? CategoryIconAlt = null, string? AccountIconPath = null, string? AccountIconAlt = null, string? TargetAccountIconPath = null, string? TargetAccountIconAlt = null);
...
public record PlannerMonth(DateOnly Month, DateOnly Today, IReadOnlyList<PlannerEvent> Events, IReadOnlyList<PlannerAccountBalance> Accounts, IReadOnlyList<PlannerCurrencySummary> Totals, IReadOnlyList<PlannerBalancePoint> Points, bool IsClosed = false, bool HasSnapshot = true, string? Notes = null);
public record PlannerNotesInput(string? Notes);
...
public interface IPlannerService
{
    ...
    Task UpdateNotesAsync(int year, int month, PlannerNotesInput input, CancellationToken cancellationToken);
    ...
    Task SetPausedAsync(Guid id, bool isPaused, CancellationToken cancellationToken);
}

public record PlannerPauseInput(bool IsPaused);
