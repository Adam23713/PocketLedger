...
public class PlannerHistoryTests
{
    ...
    public async Task PausePersistsAcrossMonthsAndResumeDoesNotChangeClosedHistory()
    {
        ...
        var copy = Assert.Single(october.Events, item => item.Source == PlannerEventSource.Planned);
        ...
        Assert.True(Assert.Single(september.Events, item => item.Source == PlannerEventSource.Planned).IsPaused);
